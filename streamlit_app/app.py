...
if st.session_state.screen == "upload":
    ...
    if uploaded_file is not None:
        ...
        if raw_preview is not None:
            ...
            matched_required = [c for c in REQUIRED_COLS if guesses.get(c) is not None]

            if unmatched_required:
                st.error(
                    f"❌ Couldn't find a column for: **{', '.join(unmatched_required)}**. "
                    f"Rename that column in your CSV (or use one of the recognized aliases below) and re-upload."
                )
                with st.expander("Recognized column names for each required field"):
                    for c in REQUIRED_COLS:
                        aliases = ", ".join(f"`{a}`" for a in [c] + COLUMN_ALIASES.get(c, []))
                        st.markdown(f"**{c}**: {aliases}")
            else:
                match_summary = " · ".join(f"`{guesses[c]}` → {c}" for c in REQUIRED_COLS)
                st.success(f"✅ Columns matched — {match_summary}")
                ready_to_run = True

    run_clicked = st.button("🚀 Run Analysis", type="primary", use_container_width=True, disabled=not ready_to_run)
    ...
    with st.expander("📋 What columns does my file need?"):
        st.caption("Different names? No problem — common aliases are auto-detected (e.g. \"Order ID\" → InvoiceNo, \"Order Date\" → InvoiceDate) before the file is validated.")
    ...
    if run_clicked and uploaded_file is not None:
        ...
        for canonical in COLUMN_ALIASES:
            if canonical in raw_df.columns and guesses.get(canonical) != canonical:
